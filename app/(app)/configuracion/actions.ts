...
export async function updateDoctorProfileAction(_prev: ActionState, formData: FormData): Promise<ActionState> {
  ...
  const update: Record<string, unknown> = {
    ...
    procedimiento: {
      ...
      pre_secciones: JSON.parse((formData.get('pre_secciones') as string) || '[]'),
      pre_prep_inicio: Number(formData.get('pre_prep_inicio') || '-6'),
      pre_prep_fin: Number(formData.get('pre_prep_fin') || '-4'),
      postquirurgico_mostrar: formData.get('postquirurgico_mostrar') === 'on',
      postquirurgico_label: (formData.get('postquirurgico_label') as string)?.trim() || 'Instrucciones postquirúrgicas',
      postquirurgico_secciones: JSON.parse((formData.get('postquirurgico_secciones') as string) || '[]'),
      postcuidados_mostrar: formData.get('postcuidados_mostrar') === 'on',
      postcuidados_label: (formData.get('postcuidados_label') as string)?.trim() || 'Cuidados post',
      postcuidados_secciones: JSON.parse((formData.get('postcuidados_secciones') as string) || '[]'),
    },
